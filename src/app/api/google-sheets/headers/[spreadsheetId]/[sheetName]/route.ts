...
export async function GET(
  ...
  try {
    ...
    // Fetch the first row (headers) from the specified sheet
    const range = `'${decodedSheetName}'!1:1`;
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: "Failed to fetch headers from Google Sheets", details: errorText },
        { status: response.status }
      );
    }
    ...
    // Filter out empty headers and ensure they're strings
    const cleanHeaders = headers
      .filter((header: unknown) => header && typeof header === 'string' && header.trim())
      .map((header: string) => header.trim());

    return NextResponse.json({ headers: cleanHeaders });
  } catch (error) {
